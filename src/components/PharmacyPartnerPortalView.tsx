          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-amber-700">
                You are currently viewing as patient / guest. Log in with your Pharmacy License or Marg POS Key to manage real-time inventory.
              </div>
            </div>
          </div>
          {onOpenLoginModal && (
      ...
      <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2 text-xs mb-1">
            ...
            <span className="text-gray-500 font-medium flex items-center gap-1">
              <RefreshCw className={`w-3 h-3 text-[#007952] ${isSyncing ? 'animate-spin' : ''}`} />
              Marg ERP / ABDM Sync: <strong>{lastSyncText}</strong>
            </span>
            {isPharmacist && (
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            disabled={isSyncing}
            className="px-3.5 py-2 border border-gray-300 rounded-xl text-xs font-semibold text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 transition-colors"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
            <span>{isSyncing ? 'Syncing POS...' : 'Sync Marg POS'}</span>
          </button>
